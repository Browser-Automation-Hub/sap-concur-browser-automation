/**
 * anchor-cloud.js — AnchorBrowser cloud example for SAP Concur
 *
...
const { login_concur, submit_expense_report } = require('../src/actions');
...
async function main() {
  ...
  try {
    log('Starting SAP Concur automation (AnchorBrowser Cloud mode)');
    ...
    const result = await withAnchorBrowser(async (page) => {
      ...
      const data1 = await login_concur(page, {});
      log('Action 1 result:', data1);

      const data2 = await submit_expense_report(page, {});
