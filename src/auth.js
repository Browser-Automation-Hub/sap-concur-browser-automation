/**
 * auth.js — Authentication handler for SAP Concur
 * Supports: SSO (Okta / Azure AD / ADFS), MFA (Okta / Azure MFA / Duo)
 *
...
const BASE_URL = process.env.CONCUR_URL || 'https://www.concursolutions.com/nui/signin';
const USERNAME = process.env.CONCUR_USERNAME;
const PASSWORD = process.env.CONCUR_PASSWORD;
...
async function performLogin(page) {
  if (!USERNAME || !PASSWORD) {
    throw new Error(`Set CONCUR_USERNAME and CONCUR_PASSWORD in .env`);
  }

  console.log('Logging in to SAP Concur...');
  await page.goto(BASE_URL, { waitUntil: 'networkidle2' });
