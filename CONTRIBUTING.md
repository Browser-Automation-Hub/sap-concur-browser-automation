# Contributing to SAP Concur Browser Automation
...
- **Broken selectors**: SAP Concur updates its UI periodically. If a selector breaks, open an issue with the current selector and what it should be.
...
2. Make your changes
3. Test your changes against a real SAP Concur instance (if you have access)
...
async function my_new_action(page, opts = {}) {
  ...
  return retry(async () => {
    ...
    await page.goto(`${process.env.CONCUR_URL}/path/to/section`);
...
When you find working selectors for SAP Concur, document them:

```javascript
// SAP Concur selectors (verified YYYY-MM-DD)
...
SAP Concur updates its UI periodically. When selectors break:
