...
[![npm](https://img.shields.io/npm/v/@browser-automation-hub%2Fsap-concur-browser-automation.svg)](https://www.npmjs.com/package/@browser-automation-hub/sap-concur-browser-automation)

> Automate SAP Concur — the reliable way to interact with Concur programmatically, with or without an official API.
...
<!-- keywords: sap concur automation, concur expense automation, concur travel automation, expense report automation, concur api alternative, corporate travel expense automation -->

## What This Is

**SAP Concur** (Travel/Expense) is notoriously difficult to automate via its official API — limited endpoints, complex authentication (Okta / Azure AD / ADFS), and browser-only workflows make traditional API integration a pain.

This project gives you a **complete browser automation scaffold** for SAP Concur using Puppeteer (self-hosted, open source) or [Anchor Browser](https://anchorbrowser.io) (cloud, managed, production-ready).

This system requires **MFA** (Okta / Azure MFA / Duo). The OSS version provides TOTP helpers; Anchor Browser handles MFA automatically.

## Quick Start

```bash
git clone https://github.com/Browser-Automation-Hub/sap-concur-browser-automation.git
cd sap-concur-browser-automation
npm install
cp .env.example .env
...
- Finance teams automating expense reconciliation
- Mass expense report processing
- Travel policy enforcement automation
- T&E data extraction for ERP integration
...
- SAP Concur account with appropriate permissions
...
```env
CONCUR_URL=https://www.concursolutions.com/nui/signin
CONCUR_USERNAME=your-username
CONCUR_PASSWORD=your-password
...
const { login_concur } = require('./src/actions');

async function main() {
  const page = await createSession();
  const result = await login_concur(page, { /* options */ });
  console.log(result);
}
...
2. **SAML SSO** (Okta / Azure AD / ADFS) — intercepts the SAML redirect and completes the IdP flow
3. **MFA / TOTP** (Okta / Azure MFA / Duo) — generates TOTP codes via `otpauth` library
4. **Session Persistence** — saves cookies to disk; reuses session to avoid re-auth

### Handling Okta / Azure MFA / Duo MFA
...
SAP Concur official API is limited, expensive to access, or requires complex authentication that makes programmatic integration impractical.
...
- Handles Okta / Azure MFA / Duo MFA automatically
...
> These CSS selectors were observed in SAP Concur web interfaces. Enterprise applications update their UIs — verify against your specific instance and submit PRs when selectors break.

> 🔍 Selector reference not yet documented for SAP Concur. [Contribute selectors via PR](https://github.com/Browser-Automation-Hub/sap-concur-browser-automation/pulls).
...
2. Document new selectors (they break when Concur updates its UI)
...
*Built with ❤️ for developers who need to automate SAP Concur without wrestling with its API limitations. Powered by [Anchor Browser](https://anchorbrowser.io) for cloud-scale automation.*
